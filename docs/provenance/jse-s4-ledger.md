...
| `src/lib/newsletter/core-proxy.ts` | `466bfb0` | REIMPLEMENT (rename) | `src/lib/newsletter/newsletter-service-client.ts` + `newsletter-canonical-contract.ts` + `newsletter-bff.ts` | REIMPLEMENT | Canonical DTOs; 10s timeout; no website/turnstile forward; sanitizer | same | OIDC (S4-D) |
