...
| Port base | `main@3bd1fe0347ace48d7a9ab8fdddb80cca066999f5` (`3bd1fe0`) |
| S4-G merge on `main` | `359ecfb` (PR #24) |
...
| S3-G/H merge on `main` | `47c911c` (PR #25); implementation `3a6b66e` |
| Draft source (do not merge) | `origin/docs/jse-s4-h-implementation-closeout@1f16ef6` — 14 commits behind current `main`, diverged, and stale on S3-G/H |
| S4-H evidence | this document, **ported** onto current `main`; **not** a production deployment SHA |

This is a **port**, not a merge of the stale S4-H branch. That branch was written against `main@359ecfb` (S4-G only) and still claimed S3-G/H were absent. Current `main` already contains S3-G/H homepage repair and the S5 planning docs. Replaying `1f16ef6` would regress those facts.
...
S4-A recorded S3-G/H as **not** on `main`. That discrepancy is **closed** by PR #25 (`47c911c`). S4-A itself is not rewritten; this closeout supersedes that gap statement.
...
| `src/app/page.tsx` | Mounts DOI hero; S3-G also mounts curated discovery on the same page |
...
| Route | Kind |
|---|---|
| `/` | Static — DOI hero (S4) + curated discovery (S3-G, PR #25) |
...
S4-G did not re-test evidence-only commits after `6871c6c`. Those commits document SHA distinction only. S3-G homepage repair (`3a6b66e`) is a later `main` descendant and is **not** claimed as the S4-G tested runtime.
...
S3-G/H live homepage is **on `main`** (`47c911c`). That does **not** transfer public-site authority and is not an S4 implementation defect.
...
- [x] Ported onto current `main` (S3-G/H + S5 docs present); stale `1f16ef6` branch not merged
